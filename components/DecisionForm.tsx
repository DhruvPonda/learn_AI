...
export const DecisionForm: React.FC<DecisionFormProps> = ({ onSubmit, isSubmitting }) => {
      ...
      <div className="min-h-screen py-12 px-4 flex justify-center items-center">
        <div className="max-w-2xl w-full space-y-8 glass-card p-8 md:p-12 rounded-3xl shadow-xl">
          <div className="text-center">
            ...
            <p className="mt-2 text-slate-500 font-medium">The Referee will build a custom framework for you.</p>
          </div>
          ...
          <div className="space-y-6">
            <textarea
              required
              rows={5}
              className="appearance-none block w-full px-5 py-4 rounded-2xl border border-slate-200 dark:border-slate-700 dark:bg-slate-900/50 dark:text-white focus:ring-4 focus:ring-primary-500/10 focus:border-primary-500 outline-none transition-all text-sm leading-relaxed"
              placeholder="Describe your situation in detail..."
              value={problemStatement}
              onChange={(e) => setProblemStatement(e.target.value)}
            />

            <div className="flex items-center justify-between p-5 bg-primary-50 dark:bg-primary-900/20 rounded-2xl border border-primary-100 dark:border-primary-900/30">
              <div className="flex items-center gap-4">
                <div 
                  className={`w-12 h-7 rounded-full p-1 cursor-pointer transition-colors duration-300 ${aiAssisted ? 'bg-primary-600' : 'bg-slate-300 dark:bg-slate-700'}`} 
                ...
                <div>
                  <span className="text-sm font-bold text-slate-800 dark:text-slate-200 block">AI-Assisted Framework</span>
                  <span className="text-[10px] text-slate-500 font-medium">Auto-suggest factors and priorities</span>
                </div>
              </div>
            </div>
            ...
            <button
              ...
              className="w-full flex justify-center py-5 px-4 rounded-2xl shadow-xl shadow-primary-500/20 text-lg font-black text-white bg-primary-600 hover:bg-primary-700 disabled:opacity-50 transition-all active:scale-[0.98]"
            >
  ...
  return (
    <div className="min-h-screen py-12 px-4 flex justify-center items-center bg-slate-50/50 dark:bg-slate-900/50">
      <div className="max-w-4xl w-full space-y-10 glass-card p-8 md:p-14 rounded-[3rem] shadow-2xl">
        <div className="flex items-center justify-between">
          <button 
            onClick={() => setPhase('intent')} 
            className="group text-sm font-bold text-slate-400 hover:text-primary-600 flex items-center gap-2 transition-colors"
          >
            <span>←</span> Back to intent
          </button>
          <div className="text-[10px] font-black text-primary-600 uppercase tracking-widest">Phase 2: Fine Tuning</div>
        </div>
        ...
        <form onSubmit={handleFinalSubmit} className="space-y-12">
            ...
            {dynamicParams.map((param) => (
              <div key={param.id} className="space-y-3">
                <div className="flex justify-between items-center">
                  <label className="text-[11px] font-black uppercase text-slate-400 dark:text-slate-500 tracking-widest">{param.label}</label>
                  <span className="text-xs font-black text-primary-600">{param.value} {param.unit || ''}</span>
                </div>
                ...
                {param.type === 'toggle' && (
                  <button
                    ...
                    className={`w-full py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all border-2 ${
                      param.value ? 'bg-primary-600 text-white border-primary-600' : 'bg-transparent border-slate-200 text-slate-400'
                    }`}
                  >
                ...
                {param.type === 'select' && (
                  <select
                    ...
                    className="w-full px-4 py-3 rounded-xl bg-white dark:bg-slate-900 border-2 border-slate-100 text-xs font-bold outline-none focus:border-primary-500"
                  >
                    {param.options?.map(opt => <option key={opt} value={opt}>{opt}</option>)}
                  </select>
                )}

                <p className="text-[10px] text-slate-500 italic leading-relaxed">{param.reason}</p>
              </div>
            ))}
          </div>

          <div className="pt-10 border-t border-slate-100 dark:border-slate-800">
            <label className="block text-[11px] font-black uppercase text-slate-400 mb-4 tracking-widest">Core Priorities</label>
          ...
          <button
            ...
            className="w-full py-6 rounded-3xl text-xl font-black text-white bg-primary-600 hover:bg-primary-700 transition-all flex items-center justify-center gap-4"
          >
